...
function rnd(arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        var stkTxt = isOut ? '<span class="text-red-500 font-bold block text-[10px] uppercase tracking-wider mt-0.5">Agotado</span>' : '<span class="text-gray-500 block text-[10px] font-mono mt-0.5">STOCK: ' + p.stock + ' UDS</span>';
        
        var btnHtml = isOut ? 
            `<button class="w-full bg-[#121215] text-gray-600 font-bold text-[10px] uppercase tracking-widest py-3 rounded-lg border border-white/5 cursor-not-allowed" disabled>Pieza Agotada</button>` : 
            ...
            <div id="card-${p.id}" class="card p-4 space-y-4 flex flex-col justify-between">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-white font-bold text-sm font-mono">$${p.price}.00</span>
                        ${stkTxt}
                    </div>
...
function upUI() { 
    ...
    for (var i = 0; i < cart.length; i++) { 
        ...
        c.innerHTML += `
            <div class="flex gap-3 bg-white/[0.01] border border-white/5 p-3 rounded-xl items-center text-xs">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-white font-mono text-[11px] font-bold pt-0.5">$${item.price} x${item.q}</p>
                </div>
                <button onclick="rem(${item.id},'${item.size}','${item.color}',${item.q})" class="text-gray-600 hover:text-white px-2 transition-all"><i class="fas fa-trash text-xs"></i></button>
            </div>`; 
    } 
...
function openCheckout() {
    ...
    formContainer.innerHTML = `
        <div class="space-y-4">
            <div class="bg-white/[0.01] border border-white/5 rounded-xl p-3 flex justify-between items-center">
                <span class="text-[9px] uppercase tracking-widest text-gray-500 font-bold font-mono">Folio de Orden:</span>
                <span id="p-folio-txt" class="text-xs font-mono font-bold text-white tracking-wider">${folioActual}</span>
            </div>
        ...
        <div class="space-y-3 pt-3 border-t border-white/5">
            <span class="text-[9px] uppercase tracking-widest text-gray-500 font-bold font-mono block">2. Datos de Transferencia Directa</span>
            <div class="bg-black/40 border border-white/5 rounded-xl p-4 font-mono text-[11px] space-y-2.5">
                ...
                <div class="flex justify-between items-center border-t border-white/5 pt-2"><span class="text-gray-500">Cuenta CLABE:</span><span class="text-white font-bold tracking-wider select-all">722969010522000447</span></div>
        ...
        <div class="pt-3 border-t border-white/5">
            <button type="submit" class="w-full bg-white text-black hover:bg-[#e11d48] hover:text-white font-bold text-xs uppercase tracking-widest py-3.5 rounded-lg transition-all shadow-md">Compartir Datos y Enviar Pedido</button>
        </div>
    `;
    togA(0); togM('pay', 1); 
}
...
function answerBot(cleanText, rawText) {
    var m = document.getElementById('c-msg'); m.innerHTML += `<div class="flex justify-end mb-3"><div class="bg-white text-black p-2.5 rounded-xl rounded-tr-none max-w-[85%] font-bold text-xs">${rawText}</div></div>`; m.scrollTop = m.scrollHeight;
    ...
    setTimeout(function() {
        ...
        var btnAction = `<button onclick="window.location.href='#catalogo'; togCh();" class="mt-2 w-full bg-[#121214] hover:bg-white hover:text-black text-white text-[10px] font-bold py-2 rounded-lg border border-white/5 uppercase">⚡ Ver Colección</button>`;
